...
class TestCudfGraph:
    ...
    def test_graph(self, edge_data_with_cudf_edges: EdgeData):
        graph = CudfGraph(edge_data_with_cudf_edges.get_source_target_amount(cudf=True))

        assert len(graph.get_graph().edges().to_pandas()) == \
               len(edge_data_with_cudf_edges.get_source_target_amount())
